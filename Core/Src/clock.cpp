...
void clock::GetNowTime() {
    uint32_t rtc_day, rtc_hr, rtc_min, rtc_sec;

    const uint32_t SECS_PER_MIN = 60;
    const uint32_t SECS_PER_HR = 60 * SECS_PER_MIN;
    const uint32_t SECS_PER_DAY = 24 * SECS_PER_HR;

    rtc_sec = RTC_GetSecond();

    rtc_day = rtc_sec / SECS_PER_DAY;
    rtc_sec -= rtc_day * SECS_PER_DAY;

    rtc_hr = rtc_sec / SECS_PER_HR;
    rtc_sec -= rtc_hr * SECS_PER_HR;

    rtc_min = rtc_sec / SECS_PER_MIN;
    rtc_sec -= rtc_min * SECS_PER_MIN;

    if (rtc_day >= 1) {
        day = day + 1;
        RTC_Init(MXC_RTC, rtc_sec - 24 * 60 * 60, 0, &sys_cfg);
        RTC_EnableRTCE(MXC_RTC);
    }
    ...
    // 万年历算法：（年+年/4+年/400-年/100+月日天数-1）/7＝XX……余星期几
    weekday = (year + year / 4 + year / 400 - year / 100
               + (leap ? dayRedress_leap[month - 1] :
                  dayRedress_common[month - 1]) + day - 1) % 7;
    hour = rtc_hr;
    minute = rtc_min;
    second = rtc_sec;
}

void clock::SolveTimeString(char *timeString) {
    // 上位机发送数据为YYYY-MM-DD-HH-MM-SS
    year = atoi(timeString);
    month = atoi(timeString + 5);
    day = atoi(timeString + 8);
    hour = atoi(timeString + 11);
    minute = atoi(timeString + 14);
    second = atoi(timeString + 17);
    RTC_DisableRTCE(MXC_RTC);
    RTC_Init(MXC_RTC, hour * 60 * 60 + minute * 60 + second, 0, &sys_cfg);
    RTC_EnableRTCE(MXC_RTC);
    // 更新星期数和闰年
    GetNowTime();
}
